...
public class SiteBean implements SpreadSheetInterface {
    public int siteId = 0;
    SiteObject selectedSiteObj = new SiteObject();
	...
	if ( saveProfile == null ||
	    ...
	    if ( siteId != 0 ) // Display the selected site
		selectedSiteObj = siteIf.getSite(siteId);
	}
	else {
		...
		if ( siteId != 0 ) {
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.SITE_NAME_STR));
		    selectedSiteObj.setSiteName(buf);
		    buf = Util.trim(valuepairs.get(AppConstants.SITE_URL_STR));
		    selectedSiteObj.setSiteUrl(buf);
		    DebugHandler.debug("Modifying Site Object " + selectedSiteObj);
		    siteIf.updateSite(selectedSiteObj);
		}
		else {
		    String buf = "";
		    Date date = null;
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.SITE_NAME_STR));
		    siteObj.setSiteName(buf);
		    buf = Util.trim(valuepairs.get(AppConstants.SITE_URL_STR));
		    siteObj.setSiteUrl(buf);
		    DebugHandler.debug("Adding Site Object " + siteObj);
		    siteIf.addSite(siteObj);
		}
	    }
	    else {
    ...
    public String getSiteInfo() throws AppException {
	...
	SiteObject siteObj = siteIf.getSite(siteId);

	if ( siteObj == null )
	    siteObj = new SiteObject();
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.CURRENT_SITE_LABEL);
	...
	SiteObject[] siteArr = siteIf.getAllSites();
	nameArrayList.add(AppConstants.NEW_SITE);
	valueArrayList.add(new Integer(0));
	for (int iterator = 0; iterator < siteArr.length; iterator++) {
	    SiteObject siteObject = siteArr[iterator];
	    if ( siteObject == null )
		break;
	    nameArrayList.add(siteObject.getSiteName());
	    valueArrayList.add(new Integer(siteObject.getSiteId()));
	}
	se = new SelectElement(AppConstants.SITE_ID_STR, nameArrayList, valueArrayList, String.valueOf(siteId), 0);
	...
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.SITE_NAME_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( siteId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.SITE_NAME_STR, selectedSiteObj.getSiteName()));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.SITE_NAME_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);

	tr = new TableRowElement();
	be = new BoldElement(AppConstants.SITE_URL_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( siteId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.SITE_URL_STR, selectedSiteObj.getSiteUrl()));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.SITE_URL_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);
	...
	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.SITE_NAME_LABEL);

	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.SITE_URL_LABEL);

	SiteInterface siteIf = new SiteImpl();
	SiteObject[] siteArr = siteIf.getAllSites();
	if ( siteArr != null && siteArr.length > 0 ) {
	    for (int iterator = 0; iterator < siteArr.length; iterator++) {
		SiteObject siteObj = siteArr[iterator];
		if ( siteObj == null )
		    break;
		rowNum++;
		col = 0;
		...
		cell.setCellValue(siteObj.getSiteId());
		...
		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(siteObj.getSiteName());

		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(siteObj.getSiteUrl());
	    }
	}
	try {
	    wb.write(fileOut);
	...
	while ( true ) {
		...
		try {
		    siteObject.setSiteId((int)cell.getNumericCellValue());
		} catch (NumberFormatException nfe) {
		...
		siteObject = siteIf.getSite(siteObject.getSiteId());
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    siteObject.setSiteName(Util.trim(cell.getStringCellValue()));
		cell = row.getCell((short)col++);
		if ( cell != null )
		    siteObject.setSiteUrl(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Updating Site " + siteObject);
		siteIf.updateSite(siteObject);
		...
		if ( cell != null )
		    siteObject.setSiteUrl(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Adding Site " + siteObject);
		siteIf.addSite(siteObject);
