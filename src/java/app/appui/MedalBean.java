...
public class MedalBean implements SpreadSheetInterface {
    public int medalId = 0;
    MedalObject selectedMedalObj = new MedalObject();
	...
	try {
	    medalId = Integer.parseInt(valuepairs.get(AppConstants.MEDAL_ID_STR));
	...
	if ( saveProfile == null ||
	    ...
	    if ( medalId != 0 ) // Display the selected medal
		selectedMedalObj = medalIf.getMedal(medalId);
	}
	else {
		...
		if ( medalId != 0 ) {
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.MEDAL_NAME_STR));
		    selectedMedalObj.setMedalName(buf);
		    buf = Util.trim(valuepairs.get(AppConstants.MEDAL_RANK_STR));
		    selectedMedalObj.setMedalRank(Integer.parseInt(buf));
		    DebugHandler.debug("Modifying Medal Object " + selectedMedalObj);
		    medalIf.updateMedal(selectedMedalObj);
		}
		else {
		    String buf = "";
		    Date date = null;
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.MEDAL_NAME_STR));
		    medalObj.setMedalName(buf);
		    buf = Util.trim(valuepairs.get(AppConstants.MEDAL_RANK_STR));
		    medalObj.setMedalRank(Integer.parseInt(buf));
		    DebugHandler.debug("Adding Medal Object " + medalObj);
		    medalIf.addMedal(medalObj);
		}
	    }
	    else {
    ...
    public String getMedalInfo() throws AppException {
	...
	MedalObject medalObj = medalIf.getMedal(medalId);

	if ( medalObj == null )
	    medalObj = new MedalObject();
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.CURRENT_MEDAL_LABEL);
	...
	MedalObject[] medalArr = medalIf.getAllMedals();
	...
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.MEDAL_NAME_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( medalId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.MEDAL_NAME_STR, selectedMedalObj.getMedalName()));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.MEDAL_NAME_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);

	tr = new TableRowElement();
	be = new BoldElement(AppConstants.MEDAL_RANK_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( medalId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.MEDAL_RANK_STR, String.valueOf(selectedMedalObj.getMedalRank())));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.MEDAL_RANK_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);
	...
	cell.setCellValue(AppConstants.MEDAL_ID_LABEL);
	...
	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.MEDAL_NAME_LABEL);

	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.MEDAL_RANK_LABEL);

	MedalInterface medalIf = new MedalImpl();
	MedalObject[] medalArr = medalIf.getAllMedals();
	if ( medalArr != null && medalArr.length > 0 ) {
	    for (int iterator = 0; iterator < medalArr.length; iterator++) {
		MedalObject medalObj = medalArr[iterator];
		if ( medalObj == null )
		    break;
		rowNum++;
		col = 0;
		...
		cell.setCellValue(medalObj.getMedalId());
		...
		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(medalObj.getMedalName());

		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(medalObj.getMedalRank());
	    }
	}
	try {
	    wb.write(fileOut);
	...
	while ( true ) {
		...
		try {
		    medalObject.setMedalId((int)cell.getNumericCellValue());
		} catch (NumberFormatException nfe) {
		...
		medalObject = medalIf.getMedal(medalObject.getMedalId());
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    medalObject.setMedalName(Util.trim(cell.getStringCellValue()));
		cell = row.getCell((short)col++);
		if ( cell != null )
		try {
		    medalObject.setMedalRank((int)cell.getNumericCellValue());
		} catch (NumberFormatException nfe) {
		    medalObject.setMedalRank(0);
		}
		DebugHandler.fine("Updating Medal " + medalObject);
		medalIf.updateMedal(medalObject);
		...
		DebugHandler.fine("Adding Medal " + medalObject);
		medalIf.addMedal(medalObject);
		...
		medalObject = medalIf.getMedal(medalObject.getMedalId());
		medalIf.deleteMedal(medalObject);
