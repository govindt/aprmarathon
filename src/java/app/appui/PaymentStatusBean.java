...
public class PaymentStatusBean implements SpreadSheetInterface {
    public int paymentStatusId = 0;
    PaymentStatusObject selectedPaymentStatusObj = new PaymentStatusObject();
	...
	try {
	    paymentStatusId = Integer.parseInt(valuepairs.get(AppConstants.PAYMENT_STATUS_ID_STR));
	...
	if ( saveProfile == null ||
	    ...
	    if ( paymentStatusId != 0 ) // Display the selected paymentstatus
		selectedPaymentStatusObj = paymentStatusIf.getPaymentStatu(paymentStatusId);
	}
	else {
		...
		if ( paymentStatusId != 0 ) {
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.PAYMENT_STATUS_NAME_STR));
		    selectedPaymentStatusObj.setPaymentStatusName(buf);
		    DebugHandler.debug("Modifying PaymentStatus Object " + selectedPaymentStatusObj);
		    paymentStatusIf.updatePaymentStatus(selectedPaymentStatusObj);
		}
		else {
		    String buf = "";
		    Date date = null;
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.PAYMENT_STATUS_NAME_STR));
		    paymentStatusObj.setPaymentStatusName(buf);
		    DebugHandler.debug("Adding PaymentStatus Object " + paymentStatusObj);
		    paymentStatusIf.addPaymentStatus(paymentStatusObj);
		}
	    }
	    else {
    ...
    public String getPaymentStatusInfo() throws AppException {
	...
	PaymentStatusObject paymentStatusObj = paymentStatusIf.getPaymentStatu(paymentStatusId);

	if ( paymentStatusObj == null )
	    paymentStatusObj = new PaymentStatusObject();
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.CURRENT_PAYMENTSTATUS_LABEL);
	...
	PaymentStatusObject[] paymentStatusArr = paymentStatusIf.getAllPaymentStatus();
	...
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.PAYMENT_STATUS_NAME_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( paymentStatusId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.PAYMENT_STATUS_NAME_STR, selectedPaymentStatusObj.getPaymentStatusName()));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.PAYMENT_STATUS_NAME_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);
	...
	cell.setCellValue(AppConstants.PAYMENT_STATUS_ID_LABEL);
	...
	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.PAYMENT_STATUS_NAME_LABEL);

	PaymentStatusInterface paymentstatusIf = new PaymentStatusImpl();
	PaymentStatusObject[] paymentstatusArr = paymentstatusIf.getAllPaymentStatus();
	if ( paymentstatusArr != null && paymentstatusArr.length > 0 ) {
	    for (int iterator = 0; iterator < paymentstatusArr.length; iterator++) {
		PaymentStatusObject paymentstatusObj = paymentstatusArr[iterator];
		if ( paymentstatusObj == null )
		    break;
		rowNum++;
		col = 0;
		...
		cell.setCellValue(paymentstatusObj.getPaymentStatusId());
		...
		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(paymentstatusObj.getPaymentStatusName());
	    }
	}
	try {
	    wb.write(fileOut);
	...
	while ( true ) {
	    ...
	    if ( row == null )
		break;
	    paymentstatusObject = new PaymentStatusObject();
		...
		try {
		    paymentstatusObject.setPaymentStatusId((int)cell.getNumericCellValue());
		} catch (NumberFormatException nfe) {
		...
		paymentstatusObject = paymentstatusIf.getPaymentStatu(paymentstatusObject.getPaymentStatusId());
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    paymentstatusObject.setPaymentStatusName(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Updating PaymentStatus " + paymentstatusObject);
		paymentstatusIf.updatePaymentStatus(paymentstatusObject);
	    } else if ( dbOp != null && dbOp.equalsIgnoreCase("INSERT") ) {
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    paymentstatusObject.setPaymentStatusName(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Adding PaymentStatus " + paymentstatusObject);
		paymentstatusIf.addPaymentStatus(paymentstatusObject);
		...
		paymentstatusObject = paymentstatusIf.getPaymentStatu(paymentstatusObject.getPaymentStatusId());
		paymentstatusIf.deletePaymentStatus(paymentstatusObject);
