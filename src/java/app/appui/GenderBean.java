...
public class GenderBean implements SpreadSheetInterface {
    public int genderId = 0;
    GenderObject selectedGenderObj = new GenderObject();
	...
	try {
	    genderId = Integer.parseInt(valuepairs.get(AppConstants.GENDER_ID_STR));
	...
	if ( saveProfile == null ||
	    ...
	    if ( genderId != 0 ) // Display the selected gender
		selectedGenderObj = genderIf.getGender(genderId);
	}
	else {
		...
		if ( genderId != 0 ) {
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.GENDER_NAME_STR));
		    selectedGenderObj.setGenderName(buf);
		    DebugHandler.debug("Modifying Gender Object " + selectedGenderObj);
		    genderIf.updateGender(selectedGenderObj);
		}
		else {
		    String buf = "";
		    Date date = null;
		    ...
		    buf = Util.trim(valuepairs.get(AppConstants.GENDER_NAME_STR));
		    genderObj.setGenderName(buf);
		    DebugHandler.debug("Adding Gender Object " + genderObj);
		    genderIf.addGender(genderObj);
		}
	    }
	    else {
    ...
    public String getGenderInfo() throws AppException {
	...
	GenderObject genderObj = genderIf.getGender(genderId);

	if ( genderObj == null )
	    genderObj = new GenderObject();
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.CURRENT_GENDER_LABEL);
	...
	GenderObject[] genderArr = genderIf.getAllGenders();
	...
	tr = new TableRowElement();
	be = new BoldElement(AppConstants.GENDER_NAME_LABEL);
	be.setId(Constants.BODY_ROW_STYLE);
	td = new TableDataElement(be);
	tr.add(td);
	if ( genderId != 0 )
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.GENDER_NAME_STR, selectedGenderObj.getGenderName()));
	else
	    td = new TableDataElement(new InputElement(InputElement.TEXT, AppConstants.GENDER_NAME_STR, Constants.EMPTY));
	tr.add(td);
	te.add(tr);
	...
	cell.setCellValue(AppConstants.GENDER_ID_LABEL);
	...
	cell = row.createCell((short)col++);
	cell.setCellStyle(cellstyleTblHdr);
	cell.setCellValue(AppConstants.GENDER_NAME_LABEL);

	GenderInterface genderIf = new GenderImpl();
	GenderObject[] genderArr = genderIf.getAllGenders();
	if ( genderArr != null && genderArr.length > 0 ) {
	    for (int iterator = 0; iterator < genderArr.length; iterator++) {
		GenderObject genderObj = genderArr[iterator];
		if ( genderObj == null )
		    break;
		rowNum++;
		col = 0;
		...
		cell.setCellValue(genderObj.getGenderId());
		...
		cell = row.createCell((short)col++);
		cell.setCellStyle(cellstyleTblLeft);
		cell.setCellValue(genderObj.getGenderName());
	    }
	}
	try {
	    wb.write(fileOut);
	...
	while ( true ) {
		...
		try {
		    genderObject.setGenderId((int)cell.getNumericCellValue());
		} catch (NumberFormatException nfe) {
		...
		genderObject = genderIf.getGender(genderObject.getGenderId());
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    genderObject.setGenderName(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Updating Gender " + genderObject);
		genderIf.updateGender(genderObject);
	    } else if ( dbOp != null && dbOp.equalsIgnoreCase("INSERT") ) {
		col = 2; // Starting from 3rd Column
		cell = row.getCell((short)col++);
		if ( cell != null )
		    genderObject.setGenderName(Util.trim(cell.getStringCellValue()));
		DebugHandler.fine("Adding Gender " + genderObject);
		genderIf.addGender(genderObject);
		...
		genderObject = genderIf.getGender(genderObject.getGenderId());
		genderIf.deleteGender(genderObject);
