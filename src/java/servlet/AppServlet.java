...
public class AppServlet extends HttpServlet {
			...
			HttpServletResponse res) throws ServletException, IOException 
    {
		...
		if ( firstTime ) {
			...
			SiteObject sObj = new SiteObject();
			sObj.setSiteUrl(req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort());
			DebugHandler.fine(sObj);
			try {
				ArrayList<SiteObject> sObjArr = sif.getSites(sObj);
				DebugHandler.fine(sObjArr);
				sObj = sObjArr.get(0); // Only one site should match URL
				AppConstants.SITE_ID = sObj.getSiteId();
				firstTime = false;
			} catch (AppException ae) {
				DebugHandler.severe("Exception getting site info");
			}
		}
		@SuppressWarnings("unchecked")
