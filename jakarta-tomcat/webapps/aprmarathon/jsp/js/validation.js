...
// Removes end (trailing) whitespace characters from s.
// Global variable whitespace (see above)
// defines which characters are considered whitespace.
...
{   var i = s.length - 1;
    ...
    while (i >= 0) {
		if ( ! charInString (s.charAt(i), whitespace)) { // If not a whitespace
			break;
		}
		i--;
	}
...
{   return reUpperCaseLetter.test(c)
}
...
function IsBug(s) {
	...
	for (i = 0; i < string_arr.length; i++ ) {
		if ( ! isWhitespace(string_arr[i]) ) {
			if ( ! reBugId.test(string_arr[i]) ) {
				return false;
			}
		}
	}
